...
class CalendarView(QWidget):
    ...
    def __init__(
        self, store: CalendarStore, settings: SettingsManager, parent: QWidget | None = None
    ) -> None:
        ...
        self._month_view = MonthlyCalendarPage(store, settings)
        self._week_view = WeeklyCalendarPage(store, settings)
        ...
        self._play_fade_in()
        # przy aktywnej persystencji (Supabase) nie ładujemy automatycznie źródeł,
        # żeby nie zdublować wydarzeń po synchronizacji zdalnej
        if not getattr(store, "_persistence", None):
            self.reload_external_sources(initial=True)
    ...
    def _build_toolbar(self) -> QFrame:
        ...
        for index, label in enumerate(["Miesiac", "Tydzien"]):
            button = QToolButton()
    ...
    def _import_ics(self) -> None:
        ...
        try:
            self._logger.info(
                "calendar_import_start",
                extra={"event": "calendar_import_start", "source_type": source_type, "source": source_ref},
            )
            if mode == "url":
                source_type = "url"
                source_ref = value.strip()
                try:
                    data = request.urlopen(source_ref, timeout=15).read()
                except urlerror.URLError as exc:  # type: ignore[assignment]
                    QMessageBox.warning(self, "Import nieudany", f"Nie mozna pobrac danych: {exc}")
                    return
            ...
            source_id = self._source_key({"type": source_type, "path": source_ref})
            # wyczyść poprzednie wydarzenia z tego źródła, aby uniknąć duplikatów
            self._store.remove_source_events(source_id)
            imported = self._store.import_ics(target_path, source_id=source_id)
            self._loaded_sources.add(source_id)

            display_name = Path(source_ref).stem if source_type == "file" else source_ref
            self._settings.add_calendar_source(display_name, source_ref, source_type=source_type)
        except Exception as exc:  # noqa: BLE001
            self._logger.error(
                "calendar_import_failed",
                exc_info=exc,
                extra={"event": "calendar_import_failed", "source_type": source_type, "source": source_ref},
            )
        ...
        if imported:
            self._logger.info(
                "calendar_import_finished",
                extra={
                    "event": "calendar_import_finished",
            ...
            QMessageBox.information(
                self,
                "Import zakonczony",
                f"Zaimportowano {imported} wydarzen.",
            )
        else:
            self._logger.info(
                "calendar_import_empty",
                extra={"event": "calendar_import_empty", "source_type": source_type, "source": source_ref},
            )
            QMessageBox.information(
                self,
                "Brak nowych wydarzen",
                "Nie znaleziono wydarzen do importu.",
            )

    def _source_key(self, entry: Dict[str, str]) -> str:
        source_type = entry.get("type", "file") or "file"
        path = entry.get("path", "")
        return f"{source_type}:{path}"

    def _import_external_entry(self, entry: Dict[str, str], source_id: str) -> bool:
        source_type = entry.get("type", "file") or "file"
        ...
        temp_path: Optional[str] = None
        try:
            if source_type == "url" or path.lower().startswith("http"):
                data = request.urlopen(path, timeout=15).read()
                with tempfile.NamedTemporaryFile(delete=False, suffix=".ics") as tmp:
                    tmp.write(data)
                    temp_path = tmp.name
                target_path = temp_path
            ...
            if temp_path and os.path.exists(temp_path or ""):
                os.remove(temp_path)

    def reload_external_sources(self, initial: bool = False) -> None:
        if initial and getattr(self._store, "_persistence", None):
            return
        sources = self._settings.get_calendar_sources()
        desired = {self._source_key(entry) for entry in sources}

        for key in list(self._loaded_sources):
            if key not in desired:
                self._store.remove_source_events(key)
                self._loaded_sources.remove(key)

        for entry in sources:
            key = self._source_key(entry)
            if key in self._loaded_sources:
                continue
            if self._import_external_entry(entry, key):
                self._loaded_sources.add(key)
    ...
    def _add_event(self) -> None:
        ...
        try:
            ...
            QMessageBox.warning(self, "Bad danych", str(exc))
            return
...
class MonthlyCalendarPage(QWidget):
    ...
    def __init__(
        ...
        self._calendar = EventCalendarWidget(store, settings)
    ...
    def _populate_events(self, target_day: date) -> None:
        ...
        if not events:
            placeholder = QListWidgetItem("Brak wydarzen")
...
class EventCalendarWidget(QCalendarWidget):
    def __init__(
        ...
        for button, icon_name in (
            (prev_button, "chevron_left.svg"),
            (next_button, "chevron_right.svg"),
        ):
...
class WeeklyCalendarPage(QWidget):
    ...
    def __init__(
        ...
        self._prev_button = QPushButton(" Poprzedni tydzien")
        self._prev_button.clicked.connect(self._go_previous_week)
        self._next_button = QPushButton("Nastepny tydzien ")
    ...
    def refresh(self) -> None:
        ...
        for index, section in self._day_sections.items():
            ...
            day_name = polish_locale.dayName(
                QDate(day_date.year, day_date.month, day_date.day).dayOfWeek(),
...
class EventDialog(QDialog):
    def __init__(self, parent: QWidget | None = None, event: Optional[Event] = None) -> None:
        ...
        if event is not None:
            self._delete_button = QPushButton("Usun")
    ...
    def _apply_defaults(self) -> None:
        ...
        current_index = next(
            (
                index
                for index, (name, _) in enumerate(COLOR_PRESETS)
                if name == self._event.color_key
            ),
            0,
        )
    ...
    def _setup_datetime_edit(self, edit: QDateTimeEdit) -> None:
        ...
        try:
            ...
            if nav_bar is not None:
                ...
                if layout is not None:
                    ...
                    def update_label(year: int | None = None, month: int | None = None) -> None:
                        ...
                        month_name = local_locale.standaloneMonthName(
                            m, QLocale.FormatType.LongFormat
                        )
            ...
            # Jesli modyfikacja UI kalendarza sie nie powiedzie, ignorujemy aby uniknac awarii.
            pass
